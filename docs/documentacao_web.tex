...
\texttt{core/state.js} & Estado compartilhado: paises selecionados, ano ativo e sistema de assinaturas. \\
\texttt{core/theme.js} & Alternancia entre tema claro e escuro, persistencia e aviso aos outros modulos. \\
\texttt{core/narrative.js} & Integracao com Scrollama, passos dos atos e navegacao ativa. \\
\texttt{core/timeline.js} & Linha visual da narrativa acompanhando os atos iniciais. \\
\texttt{core/ambient.js} & Fundo animado em canvas com particulas e globo translucido. \\
\texttt{acts/ato-1/race.js} & Ato 1: corrida IDH versus CO2 com bolhas animadas. \\
\texttt{acts/ato-2/footprint.js} & Ato 2: calculo e exibicao de ``quantas Terras''. \\
\texttt{acts/ato-3/trade-map.js} & Ato 3: mapa territorio versus consumo e barras de importadores/exportadores. \\
\texttt{acts/ato-4/facilities-map.js} & Ato 4: mapa deck.gl com instalacoes emissoras reais. \\
\texttt{acts/ato-4/treemap.js} & Ato 4: treemap navegavel por setor e instalacao. \\
\texttt{acts/ato-5/globe.js} & Ato 5: globos 3D, selecao de paises, tooltip, legenda e cameras sincronizadas. \\
\texttt{acts/ato-5/map2d.js} & Ato 5: mapa 2D leve com seletor de indicador. \\
\texttt{acts/ato-5/viewmode.js} & Alternancia entre Mapa 2D e Globos 3D no Ato 5. \\
\texttt{acts/ato-5/comparison.js} & Ato 5: painel Vega-Lite com quatro graficos coordenados. \\
\texttt{acts/ato-5/parallel.js} & Ato 5: coordenadas paralelas com filtro por brush. \\
\texttt{core/fx.js} & Utilitario simples para aplicar transicao de fade em elementos atualizados. \\
\bottomrule
\end{longtable}

\subsection{Mapa de arquivos por ato e visualizacao}

\begin{longtable}{>{\raggedright\arraybackslash}p{2.4cm}>{\raggedright\arraybackslash}p{4.2cm}>{\raggedright\arraybackslash}p{4.5cm}>{\raggedright\arraybackslash}p{3.1cm}}
\toprule
\textbf{Parte} & \textbf{Visualizacao ou funcao} & \textbf{Arquivos principais} & \textbf{Dados usados} \\
\midrule
Base & Estrutura da pagina, topbar, hero, atos e rodape. & \texttt{index.html} & Elementos HTML da pagina. \\
Base & Tema, layout, cards, responsividade e estados visuais. & \texttt{styles.css} & Variaveis CSS e classes. \\
Base & Estado compartilhado de ano e paises selecionados. & \texttt{core/state.js} & Cliques e selecoes das visualizacoes. \\
Base & Tema claro/escuro. & \texttt{core/theme.js} & \texttt{localStorage} e variaveis CSS. \\
Base & Scroll narrativo e links ativos. & \texttt{core/narrative.js} & Passos dos atos no HTML. \\
Base & Fundo animado. & \texttt{core/ambient.js} & Scroll e tema atual. \\
Base & Linha visual da narrativa. & \texttt{core/timeline.js} & Posicao dos atos na pagina. \\
Ato 1 & Corrida IDH versus CO2. & \texttt{acts/ato-1/race.js} & \texttt{dev\_planet\_narrative.json}. \\
Ato 2 & Quantas Terras. & \texttt{acts/ato-2/footprint.js} & \texttt{dev\_planet\_narrative.json}. \\
Ato 3 & Mapa territorio versus consumo e barras liquidas. & \texttt{acts/ato-3/trade-map.js} & \texttt{dev\_planet\_narrative.json} e \texttt{world.geojson}. \\
Ato 4 & Mapa de instalacoes emissoras. & \texttt{acts/ato-4/facilities-map.js} & Arquivos \texttt{facilities/*.json}. \\
Ato 4 & Treemap por setor e instalacao. & \texttt{acts/ato-4/treemap.js} & Arquivos \texttt{facilities/*.json}. \\
Ato 5 & Alternancia entre Mapa 2D e Globos 3D. & \texttt{acts/ato-5/viewmode.js} & Modo salvo no navegador. \\
Ato 5 & Mapa 2D coropletico. & \texttt{acts/ato-5/map2d.js} & \texttt{dev\_planet\_globe.json} e \texttt{world.geojson}. \\
Ato 5 & Globos 3D duais. & \texttt{acts/ato-5/globe.js} & \texttt{dev\_planet\_globe.json} e \texttt{world.geojson}. \\
Ato 5 & Painel de comparacao. & \texttt{acts/ato-5/comparison.js} & \texttt{dev\_planet\_globe.json}. \\
Ato 5 & Coordenadas paralelas. & \texttt{acts/ato-5/parallel.js} & \texttt{dev\_planet\_globe.json}. \\
Utilitario & Fade em conteudos atualizados. & \texttt{core/fx.js} & Elementos da interface. \\
\bottomrule
\end{longtable}

\subsection{Organizacao atual em pastas}

Os modulos JavaScript ficam separados por responsabilidade:
...
O arquivo \texttt{core/state.js} implementa o mecanismo de coordenacao entre visualizacoes. Ele guarda:
...
A narrativa usa Scrollama para ativar passos conforme a rolagem. Cada ato com scrollytelling tem uma area fixa com grafico e uma coluna de texto com passos. Quando um passo entra na area ativa da tela, o modulo \texttt{core/narrative.js} chama a acao correspondente:
...
O arquivo \texttt{acts/ato-1/race.js} carrega \texttt{dev\_planet\_narrative.json}, filtra os dados por ano e desenha as bolhas com escalas D3. A cada mudanca de ano, as bolhas recebem novas posicoes, tamanhos e rotulos. O quadrante verde marca a combinacao de IDH alto com baixa emissao.

O usuario pode clicar em paises para fixa-los. A selecao e enviada para \texttt{core/state.js}, permitindo que os mesmos paises aparecam nos outros atos. O grafico tambem usa transicoes suaves para o movimento temporal das bolhas.
...
A visualizacao usa HTML, CSS e JavaScript puro com dados carregados por \texttt{fetch}. O modulo responsavel e \texttt{acts/ato-2/footprint.js}.
...
O arquivo \texttt{acts/ato-3/trade-map.js} carrega o GeoJSON e os dados narrativos. O pais recebe cor pela escala sequencial \texttt{d3.interpolateYlOrRd}. Quando o usuario troca o modo, o mapa e redesenhado com o indicador correspondente.
...
O arquivo \texttt{acts/ato-4/facilities-map.js} tambem altera o enquadramento do mapa quando ha paises selecionados. Assim, a visualizacao aproxima as instalacoes dos paises escolhidos.
...
Os pontos, setores, tipos, coordenadas e emissoes vem do Climate TRACE. O mapa base vem dos tiles da CARTO. A selecao de paises vem de \texttt{core/state.js}, por isso o mapa responde a cliques feitos em outros atos.
...
O mapa 2D foi criado em \texttt{acts/ato-5/map2d.js} com D3.js. Ele usa o mesmo GeoJSON mundial e a mesma base pais-ano dos globos. A cor depende do indicador selecionado. Indicadores numericos usam escala linear; o indicador categorico de perfil desenvolvimento-carbono usa cores por categoria.
...
Os globos foram implementados em \texttt{acts/ato-5/globe.js} com Three.js, OrbitControls e three-globe. Existem dois renderizadores lado a lado:
...
O arquivo \texttt{acts/ato-5/comparison.js} usa Vega-Lite para gerar quatro graficos:
...
As coordenadas paralelas foram feitas em \texttt{acts/ato-5/parallel.js} com D3.js. Cada linha representa um pais. Os eixos representam indicadores como IDH, expectativa de vida, escolaridade, PIB e CO2 per capita.
...
O botao da topbar alterna entre tema claro e escuro. O arquivo \texttt{core/theme.js} aplica o tema no atributo \texttt{data-theme} do HTML e salva a escolha no \texttt{localStorage}. Os outros modulos leem variaveis CSS para adaptar cores de mapas, textos, linhas e fundos.

\subsection{Topbar e navegacao}

A topbar fica presa ao topo da tela com \texttt{position: sticky}. Ela contem a marca do projeto, links para os atos e o botao de tema. O modulo \texttt{core/narrative.js} usa \texttt{IntersectionObserver} para destacar o link do ato visivel.
...
O arquivo \texttt{core/fx.js} reaplica uma classe CSS de fade quando um conteudo muda. Isso evita cortes bruscos em paineis, mapas e textos atualizados.
